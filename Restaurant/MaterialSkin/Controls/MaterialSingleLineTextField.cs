...
namespace MaterialSkin.Controls
{
    ...
    public class MaterialSingleLineTextField : Panel, IMaterialControl
    {
        [Browsable(false)]
        ...
        [Browsable(true), EditorBrowsable(EditorBrowsableState.Always)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
        [Description("When a key is pressed"), Category("Key")]
            ...
            if (ThousandsSeparator && !AcceptMathChars)
            {
                ...
                if (!string.IsNullOrEmpty(Text) && e.KeyData != Keys.Left && e.KeyData != Keys.Right)
                {
                    ...
                    if (BigInteger.TryParse(Text, style, culture, out valueBefore))
                    {
                        ...
                        if (e.KeyData != Keys.Delete && e.KeyData != Keys.Back) baseTextBox.Select(Text.Length, 0);
                        else baseTextBox.Select(indexSelectionBuffer, 0);
                    }
                }
            }
        }
