...
namespace MaterialSkin.Controls
{
    ...
    public class MaterialSmallCard : Panel, IMaterialControl
    {
        ...
        public override string Text
        {
            ...
            set
            {
                // Funcionando pero WTF?
                string[] tokens = value.Split();
                string val = "";
                string temp = "";
                foreach (string token in tokens)
                {
                    if (CreateGraphics().MeasureString(temp + token, MaterialSkinManager.ROBOTO_MEDIUM_10).Width > GetPreferredSize().Width - 10)
                    {
                        val += temp + "\n";
                        temp = token + " ";
                    }
                ...
                base.Text = val;
                textSize = CreateGraphics().MeasureString(val, MaterialSkinManager.ROBOTO_MEDIUM_10);
                if (AutoSize)
